export const getDifferenceSetOfD3Contour =(d3Contour)=>{
  const cleanedShapes = d3Contour.map((v,i,arr)=>clearnShape(v?.coordinates,arr[i+1]?.coordinates,i))
  const diffD3Contour = d3Contour.map((v,i)=>Object({
    type: v.type,
    value: v.value,
    coordinates: cleanedShapes[i],
  }))
  return diffD3Contour
}
